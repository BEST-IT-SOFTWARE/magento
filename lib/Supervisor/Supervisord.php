...
class Supervisor_Supervisord {
    protected $_url = '';
    protected $_path = '/RPC2';
    protected $_host = '';
    protected $_port = '';
    protected $_haeders = array('Content-Type: text/xml');
    ...
    protected function _call($method, $params = array()) {
        ...
        if (!$response)
            throw new Exception('Invalid response from '.$this->_url. ":". $file);
    ...
    */
    ...
    public function stopProcess($name, $wait = true) {
        return $this->_call('supervisor.stopProcess', array(
            $name,
            $wait
        ));
    }
